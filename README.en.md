...
-   Recommended VSCode version `1.93` or above.
...
-   **Terminal Offset**: The terminal input box may shift downward, and the specific manifestation depends on the screen resolution. Temporary solutions:
    1. Adjust the terminal height slightly to make VSCode recalculate automatically.
    2. Switch to the Ports tab and then back to the Terminal tab.
-   **Extension Detail Page Animations**: Currently, only the first extension detail page has animation effects. This issue will be fixed in the next version.
